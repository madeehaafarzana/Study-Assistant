...
export function QuestionAccordion({
  question,
  answerSteps,
}: QuestionAccordionProps): React.ReactNode {
  ...
  return (
    <div className="bg-slate-800/50 backdrop-blur-lg rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-white/10">
      ...
      >
        <h3 className="text-md font-semibold text-slate-100 flex-1 pr-4">
          {question}
        </h3>
        <span
              ...
              {answerSteps.map((step, index) =>
                isCodeSnippet(step) ? (
                  <li key={index} className="list-none -ml-5 my-3">
                    <pre className="bg-slate-900/80 text-white p-4 rounded-md text-sm overflow-x-auto whitespace-pre-wrap break-words font-mono border border-white/10 max-w-full">
                  ...
                  <li
                    key={index}
                    className="text-slate-300 leading-relaxed list-decimal marker:text-indigo-400 marker:font-bold break-words"
                  >
                    {step}
                  </li>
                )
